...
# This is a simple example. A real implementation would use a more robust
# state store like Redis or a database.
suspicious_processes = {}  # pid -> timestamp

def create_incident_in_backend(description, severity, source_ip=None):
...
def process_event(event):
    ...
    if event_type == "process":
        ...
        # Check if a process is started from a suspicious directory
        if "/tmp" in process_data.get("cmdline", ""):
            log.warning(f"Suspicious process detected: {process_data['cmdline']} (PID: {process_data['pid']})")
        ...
        if pid in suspicious_processes:
            # Check if the connection was made shortly after the process started
            if time.time() - suspicious_processes[pid] < 60: # 1 minute window
                description = f"UEBA Anomaly: Process {pid} started from /tmp and made a network connection to {net_data['remote_address']}"
                create_incident_in_backend(description, "High", source_ip=net_data.get('local_address'))
                # Remove the process from the suspicious list to avoid duplicate alerts
                del suspicious_processes[pid]

    # Clean up old suspicious processes
    for pid, timestamp in list(suspicious_processes.items()):
        if time.time() - timestamp > 300: # 5 minute window
            del suspicious_processes[pid]
        ...
        while True:
            ...
            if msg.error():
                ...
                try:
                    ...
                    log.error(f"Failed to decode JSON message: {msg.value()}")
