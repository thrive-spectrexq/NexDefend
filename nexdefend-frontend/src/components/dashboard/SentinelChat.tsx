...
export const SentinelChat = () => {
  const [messages, setMessages] = useState([
    { role: 'ai', text: 'Sentinel AI initialized. Monitoring 42 active endpoints. No critical anomalies detected.' }
  ]);
  ...
  return (
    <GlassCard title="Sentinel Copilot" icon={<Sparkles className="h-5 w-5" />} className="h-[500px] flex flex-col">
        ...
        {messages.map((msg, i) => (
          <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[85%] p-3 rounded-lg font-mono text-sm ${
              ...
              {msg.role === 'ai' && <div className="text-xs text-accent mb-1 font-bold flex items-center gap-1"><Bot size={12}/> SENTINEL</div>}
              {msg.text}
            </div>
          </div>
        ))}
      ...
      <div className="mt-4 pt-4 border-t border-white/10 flex gap-2">
        <input
          ...
          placeholder="Ask Sentinel to scan..."
