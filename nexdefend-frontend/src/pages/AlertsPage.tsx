    ...
    const fetchAlerts = async () => {
      try {
        ...
        const mockAlerts = Array.from({ length: 50 }, (_, i) => ({
            ...
            severity: Math.random() > 0.9 ? 'critical' : Math.random() > 0.7 ? 'high' : 'medium',
            rule_name: Math.random() > 0.5 ? 'Suspicious PowerShell' : 'Port Scan Detected',
            source_ip: `192.168.1.${Math.floor(Math.random() * 20)}`,
                                ...
                                <tr key={idx} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                                    ...
                                    <td className="py-3 px-4">
                                        <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${
                                            String(alert.severity).toLowerCase().includes('critical') ? 'bg-red-500/20 text-red-400' :
